...
m1 = feols(investment_pc~ war_share_gdp + l(war_share_gdp, 1) | country+year, panel.id = ~country+year, data = data)
m1 

m2 = feols(consumptio_pc~ war_share_gdp + l(war_share_gdp, 1) | country+year, panel.id = ~country+year, data = data)
m2 
...
modelsummary(
  ...
  coef_map = c("war_share_gdp" = "Gasto en defensa (\\% del PIB)",
               "l(war_share_gdp, 1)" = "Gasto en defensa t-1 (\\% del PIB)"),
  stars = TRUE,
...
fmt = fmt_decimal(decimal.mark = ".", big.mark = ","),
 output = "03_regressions/output/03_tabla_reg_mecanismo_lag.tex"
)

# small changes

## quitar begin and end table
a = readLines("03_regressions/output/03_tabla_reg_mecanismo_lag.tex")
a = a[-c(1:2,length(a))]
...
# 04 export
#================================#
writeLines(a, "03_regressions/output/03_tabla_reg_mecanismo_lag.tex")
